...
class Schedule(BaseModel):
    ...
    def validate(self):
        if self.departure_time > self.arrival_time:
            raise ValidationError(
                "Departure time can not be more  to arrival time"
            )

        if self.actual_departure > self.actual_arrival:
            raise ValidationError(
                "Actual departure time can not be more to actual arrival time"
            )
...
class StatusHistory(BaseModel):
    ...
    def validate(self) -> str:
        if self.set_status_time > self.unset_status_time:
            raise ValidationError(
                "Registred time can not be more to derigisrated time"
            )
